package com.muslim.anees.data.model.radio

import com.muslim.anees.data.model.audio.AudioTrack

object RadioStations {
    val stations = listOf(
        RadioStation(
            "محمود خليل الحصري",
            "https://backup.qurango.net/radio/mahmoud_khalil_alhussary",
            " قارئ قرآن مصري ويعد أحد أعلام هذا المجال البارزين",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/hosary.jpg?raw=true"
        ),
        RadioStation(
            "إدريس أبكر",
            "https://backup.qurango.net/radio/idrees_abkr",
            "قارئ سعودي من مدينة جدة، اشتهر بصوته المؤثر في تلاوة القرآن الكريم",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/idris.png?raw=true"

        ),
        RadioStation(
            "سعد الغامدي",
            "https://backup.qurango.net/radio/saad_alghamdi",
            "إمام وخطيب سعودي، له تسجيلات شهيرة بتلاواته العذبة والمؤثرة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/saad.png?raw=true"
        ),
        RadioStation(
            "عبد الباسط عبد الصمد",
            "https://backup.qurango.net/radio/abdulbasit_abdulsamad_mojawwad",
            "من أشهر قراء العالم الإسلامي، تميز بصوت قوي وأداء متقن",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/abdelbaset.png?raw=true"
        ),
        RadioStation(
            "محمد صديق المنشاوي",
            "https://backup.qurango.net/radio/mohammed_siddiq_alminshawi_mojawwad",
            "أحد أشهر قراء مصر والعالم الإسلامي، تميز بأسلوب شجي وخاشع",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/menshawy.png?raw=true"
        ),
        RadioStation(
            "محمد اللحيدان",
            "https://backup.qurango.net/radio/mohammed_allohaidan",
            "قارئ سعودي يتميز بصوت خاشع وأداء مميز في تلاوة القرآن",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/layhadan.png?raw=true"
        ),
        RadioStation(
            "ماهر المعيقلي",
            "https://backup.qurango.net/radio/maher",
            "إمام وخطيب المسجد الحرام بمكة المكرمة، معروف بصوته الهادئ المؤثر",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/maher.png?raw=true"
        ),
        RadioStation(
            "ياسر الدوسري",
            "https://backup.qurango.net/radio/yasser_aldosari",
            "إمام وخطيب المسجد الحرام، يتمتع بصوت مؤثر وأداء متميز",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/dosri.png?raw=true"
        ),
        RadioStation(
            "ناصر القطامي",
            "https://backup.qurango.net/radio/nasser_alqatami",
            "إمام سعودي معروف بصوته الجميل وأسلوبه الهادئ",
            "https://github.com/Alikotb/Anees_Images/blob/main/Reciters/Nasser_Alqatami.jpg?raw=true"

        ),
        RadioStation(
            "أحمد العجمي",
            "https://backup.qurango.net/radio/ahmad_alajmy",
            "قارئ سعودي متميز بصوته العذب وأسلوبه المؤثر",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/agamy.png?raw=true"
        ),
        RadioStation(
            "حاتم فريد الواعر",
            "https://backup.qurango.net/radio/hatem_fareed_alwaer",
            "قارئ مصري عرف بصوته الهادئ والخاشع",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/hatem.png?raw=true"
        ),
        RadioStation(
            "سعود الشريم",
            "https://backup.qurango.net/radio/saud_alshuraim",
            "إمام المسجد الحرام بمكة المكرمة، له تسجيلات مؤثرة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/sherim.png?raw=true"
        ),
        RadioStation(
            "خالد الجليل",
            "https://backup.qurango.net/radio/khalid_aljileel",
            "إمام جامع الملك خالد بالرياض، معروف بتلاوته الخاشعة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/galel.png?raw=true"
        ),
        RadioStation(
            "محمد الطبلاوي",
            "https://backup.qurango.net/radio/mohammad_altablaway",
            "من كبار قراء مصر، اشتهر بأسلوبه المؤثر وصوته العذب",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/tablawy.png?raw=true"
        ),
        RadioStation(
            "محمد أيوب",
            "https://backup.qurango.net/radio/mohammed_ayyub",
            "إمام المسجد النبوي سابقًا، له تلاوات مميزة بصوت هادئ",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/ayub.png?raw=true"
        ),
        RadioStation(
            "محمد جبريل",
            "https://backup.qurango.net/radio/mohammed_jibreel",
            "قارئ مصري معروف بجمال صوته وخشوع تلاوته",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/gebril.png?raw=true"
        ),
        RadioStation(
            "أبو بكر الشاطري",
            "https://backup.qurango.net/radio/shaik_abu_bakr_al_shatri",
            "تلاوات عذبة بصوت الشيخ أبو بكر الشاطري، يتميز بجمال الأداء وجودة الترتيل.",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/shatri.png?raw=true"
        ),
        RadioStation(
            "محمود علي البنا",
            "https://backup.qurango.net/radio/mahmoud_ali__albanna_mojawwad",
            "قارئ مصري متميز بصوته الرخيم وأدائه المؤثر.",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/elbana.png?raw=true"
        ),
        RadioStation(
            "مصطفى اللاهوني",
            "https://backup.qurango.net/radio/mustafa_allahoni",
            "قارئ مصري له تلاوات جميلة بصوت مؤثر",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/mostafa.png?raw=true"
        ),
        RadioStation(
            "مصطفى رعد العزاوي",
            "https://backup.qurango.net/radio/mustafa_raad_alazawy",
            "قارئ عراقي، له تلاوات بأسلوب مميز وخاشع",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/ezawy.png?raw=true"
        ),
        RadioStation(
            "جمعان العصيمي",
            "https://backup.qurango.net/radio/jamaan_alosaimi",
            "قارئ سعودي يتميز بأداء مؤثر وخاشع",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/osaimy.png?raw=true"
        ),
        RadioStation(
            "معيض الحارثي",
            "https://backup.qurango.net/radio/moeedh_alharthi",
            "قارئ سعودي، له تلاوات متميزة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/harthy.png?raw=true"
        ),
        RadioStation(
            "أحمد الطرابلسي",
            "https://backup.qurango.net/radio/ahmed_altrabulsi",
            "قارئ كويتي، يتميز بصوت مؤثر وأداء مميز",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/trablsy.png?raw=true"
        ),
        RadioStation(
            "إذاعة أنيس",
            "https://backup.qurango.net/radio/mix",
            "باقة مختارة من أجمل وأعذب تلاوات القراء",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/tafsirquran.png?raw=true"
        ),
        RadioStation(
            "تلاوات خاشعة",
            "https://backup.qurango.net/radio/salma",
            "باقة مختارة من أجمل وأعذب تلاوات القراء",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/telawaat.png?raw=true"

        ),
        RadioStation(
            "الفتاوى",
            "https://backup.qurango.net/radio/fatwa",
            "إذاعة مختصة ببث الفتاوى الدينية المباشرة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/fatwa.png?raw=true"
        ),
        RadioStation(
            "الرُقية الشرعية",
            "https://backup.qurango.net/radio/roqiah",
            "إذاعة متخصصة في الرُقية الشرعية وآيات الشفاء",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/roqia.png?raw=true"
        ),
        RadioStation(
            "تفسير القرآن",
            "https://backup.qurango.net/radio/tafseer",
            "بث مباشر لتفسير آيات القرآن الكريم من كبار العلماء",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/tafsirquran.png?raw=true"
        ),
        RadioStation(
            "إذاعة رمضان",
            "https://backup.qurango.net/radio/ramadan",
            "إذاعة خاصة بشهر رمضان الكريم بتلاوات وابتهالات رمضانية",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/ramadan.png?raw=true"
        ),
        RadioStation(
            "ستة أيام من شوال",
            "https://backup.qurango.net/radio/SixDaysOfShawwal",
            "إذاعة مخصصة لتلاوات وأحاديث عن صيام الست من شوال",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/shawaal.png?raw=true"
        ),
        RadioStation(
            "يوم عاشوراء",
            "https://backup.qurango.net/radio/TheDayofAshoora",
            "بث مباشر لتلاوات وأحاديث عن فضل يوم عاشوراء",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/ashoraa.png?raw=true"
        ),
        RadioStation(
            "سيرة النبي محمد ﷺ",
            "https://backup.qurango.net/radio/fi_zilal_alsiyra",
            "إذاعة مختصة ببث سيرة النبي محمد ﷺ بشكل مفصل",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/sera.png?raw=true"
        ),
        RadioStation(
            "مختصر السيرة النبوية",
            "https://backup.qurango.net/radio/almukhtasar_fi_alsiyra",
            "بث مباشر للسيرة النبوية المختصرة وأحداثها المهمة",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/sera.png?raw=true"
        ),
        RadioStation(
            "عشر ذي الحجة",
            "https://backup.qurango.net/radio/ten_dhul_hijjah",
            "إذاعة مخصصة لتلاوات وأحاديث عن فضل أيام العشر",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/ashr.png?raw=true"
        ),
        RadioStation(
            "تفسير الطبري - ختمة",
            "https://backup.qurango.net/radio/tabri",
            "بث مباشر لختم تفسير القرآن الكريم من كتاب الطبري",
            "https://github.com/Alikotb/Anees_Images/blob/main/radio_img/telawaat.png?raw=true"
        )
    )
}

val audioStations = RadioStations.stations.mapIndexed { index, station ->
    AudioTrack(
        index = index,
        title = station.name,
        description = station.description,
        reciter = station.name,
        reciterImage = station.imageURL,
        typeIcon = "",
        reciterBaseUrl = "",
        uri = station.url,
    )
 }
